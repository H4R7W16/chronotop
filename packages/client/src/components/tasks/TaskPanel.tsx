...
export function TaskPanel() {
  ...
  const canAnswer = Boolean(user) || isStaticDemo;
    ...
    try {
      if (isStaticDemo) {
        setAnswers(prev => ({
          ...prev,
          [taskId]: {
            id: `demo-answer-${taskId}`,
            taskId,
            value,
            submittedAt: new Date().toISOString(),
          },
        }));
        toast.success('Antwort lokal in der Demo vorgemerkt.');
        return;
      }
  ...
  async function handleRetract(taskId: string) {
    ...
    try {
      if (isStaticDemo) {
        ...
        toast.success('Lokale Demo-Antwort zurückgezogen.');
        return;
      }
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        {isStaticDemo ? (
          <div className="bg-verdigris-50 border border-verdigris-200 rounded-md px-3 py-2 text-xs text-verdigris-800">
            Pages-Demo: Antworten werden nur lokal in diesem Browser getestet und nicht gespeichert.
          </div>
        ) : !user && (
