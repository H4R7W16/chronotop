...
import { useIsTablet, useMediaQuery } from '../../hooks/useMediaQuery.js';
import { BottomInspector } from './BottomInspector.js';
...
interface WorkbenchLayoutProps {
  ...
  inspectorTriggerKey?: string | null;
  inspectorPeekLabel?: ReactNode;
...
export function WorkbenchLayout({
  ...
  const sideInspectorVisible = inspectorVisible && inspector && !isTablet;
  const bottomInspectorVisible = inspectorVisible && inspector && isTablet;
    ...
    <div className="h-full min-h-0 flex flex-col bg-white">
      ...
      <div className="flex-1 min-h-0 flex flex-col lg:flex-row">
        <div className="flex-1 min-w-0 min-h-[18rem] lg:min-h-0 relative">
          {map}
          {bottomInspectorVisible && (
            <BottomInspector
              ariaLabel={inspectorLabel}
              triggerKey={inspectorTriggerKey ?? null}
              peekLabel={inspectorPeekLabel}
            >
              {inspector}
            </BottomInspector>
          )}
      ...
      <div
        className={`shrink-0 min-h-[7rem] max-h-[22rem] border-t border-parchment-300 bg-white ${isTablet ? '' : ''}`}
